...
playDeterministicGame p1 p2 turns
  ...
  | otherwise =
    if even turns
      then let val = newPosition diceval (fst p1)
            in playDeterministicGame (val, snd p1 + val) p2 (turns + 1)
      else let val = newPosition diceval (fst p2)
            in playDeterministicGame p1 (val, snd p2 + val) (turns + 1)
  where
...
playQuantumGame ::
  ...
  -> Int
  -> Map DPState (Int, Int)
  -> (Map DPState (Int, Int), (Int, Int))
playQuantumGame p1 p2 turns dp
  | Map.member (p1, p2, turns) dp =
    (dp, Map.findWithDefault (-1, -1) (p1, p2, turns) dp)
  | snd p1 >= 21 = (Map.insert (p1, p2, turns) (1, 0) dp, (1, 0))
  | snd p2 >= 21 = (Map.insert (p1, p2, turns) (0, 1) dp, (0, 1))
  | otherwise =
    if even turns
      then divideUniverses
             (\val m -> playQuantumGame (val, snd p1 + val) p2 (turns + 1) m)
             (fst p1)
             (p1, p2, turns)
             dp
      else divideUniverses
             (\val m -> playQuantumGame p1 (val, snd p2 + val) (turns + 1) m)
             (fst p2)
             (p1, p2, turns)
             dp

divideUniverses ::
     (Int -> Map DPState (Int, Int) -> (Map DPState (Int, Int), (Int, Int)))
  -> Int
  -> DPState
  -> Map DPState (Int, Int)
  -> (Map DPState (Int, Int), (Int, Int))
divideUniverses f pos state dp =
  (\(m, l) -> (Map.insert state l m, l))
    (foldl
       (\(m, (p1, p2)) (d1, d2, d3) ->
          ((\(mn, (p1n, p2n)) -> (mn, (p1 + p1n, p2 + p2n))) .
           f (newPosition pos (d1 + d2 + d3)))
            m)
       (dp, (0, 0))
       (triples [1, 2, 3]))
...
readPlayerPosition filename = do
  ...
  let [one, two] =
        (map (read . head . tail . splitOn ": ") . lines) content :: [Int]
  return ((one, 0), (two, 0))
