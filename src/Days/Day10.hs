...
data ParseResult = Error Char | OK | Incomplete String deriving (Show, Eq)
...
complmap =
  Map.fromList [('>', 4), ('}', 3), (']', 2), (')', 1)] :: Map Char Int
  ...
  return
    $ ( getMiddle
      ...
      . map
          ...
          . map
              (\c -> Map.findWithDefault
                0
                (Map.findWithDefault '_' c charmap)
                complmap
              )
          . (\case
              Incomplete s -> s
...
parseSubsystem (c : ctail) st
  ...
  | c `Set.member` endingChars
  = Error c
  | otherwise
  = parseSubsystem ctail (c : st)

okIfThereAreOnlyClosingFromNowOn :: String -> ParseResult
okIfThereAreOnlyClosingFromNowOn str =
  if all (`Set.member` endingChars) str then OK else Error (head str)
