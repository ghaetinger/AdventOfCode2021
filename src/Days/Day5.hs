...
firstQuestion filename = do
  ...
  let filteredVectors =
        filter (\((x0, y0), (x1, y1)) -> x0 == x1 || y0 == y1) vectors
...
countPoints vector cmap =
  (foldl (\cmapBuf point -> Map.insertWith (+) point 1 cmapBuf) cmap .
   buildLinePoints)
    vector
...
buildLinePoints ((x0, y0), (x1, y1))
  ...
  | x0 == x1 =
    (\lsy -> zip (replicate (length lsy) x0) lsy) (buildNumberList y0 y1)
  | y0 == y1 =
    (\lsx -> zip lsx (replicate (length lsx) y0)) (buildNumberList x0 x1)
...
parseLineToVector =
  (\[x, y, z, w] -> ((x, y), (z, w))) .
  map (read :: String -> Int) . concatMap (splitOn ",") . splitOn " -> "
