...
buildLinePoints ((x0, y0), (x1, y1))
  ...
  | x0 == x1 = (\lsy -> zip (replicate (length lsy) x0) lsy)
    (buildNumberList y0 y1)
  | y0 == y1 = (\lsx -> zip lsx (replicate (length lsx) y0))
    (buildNumberList x0 x1)
  | otherwise = error "This should not happen"

buildNumberList :: Int -> Int -> [Int]
buildNumberList a b | a > b     = reverse [b .. a]
                    | otherwise = [a .. b]
