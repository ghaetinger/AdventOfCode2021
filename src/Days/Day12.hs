...
filterStartRoutes =
  Map.fromList .
  map (Data.Bifunctor.second (Set.fromList . filter (/= "start") . Set.toList)) .
  Map.toList

filterEndRoutes :: Map String (Set String) -> Map String (Set String)
filterEndRoutes = Map.delete "end"

constructRoutes :: Map String (Set String) -> Set String -> String -> [[String]]
        ...
        (\rmap (orig, end) ->
           (Map.insertWith Set.union orig (Set.singleton end) .
            Map.insertWith Set.union end (Set.singleton orig))
             rmap)
        Map.empty .
      concatMap getLineConnections . lines)
       contents)
