...
runProcess n filename = do
  ...
  let occuranceTuple =
        (sortBy (\(_, a) (_, b) -> compare b a) .
         Map.toList .
         foldl
           (\m (a:b:_, n) -> (Map.insertWith (+) b n . Map.insertWith (+) a n) m)
           Map.empty .
         Map.toList)
          pairMap
  return
...
readRules filename = do
  ...
  return
    ( foldl
        (\ps (a, b) -> Map.insertWith (+) [a, b] 1 ps)
        Map.empty
        ((\st -> zip (take (length st - 1) st) (tail st)) s)
    , Map.fromList $ map ((\(a:b:_) -> (a, head b)) . splitOn " -> ") ruleStrs)
