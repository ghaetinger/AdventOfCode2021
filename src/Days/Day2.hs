...
data Position = SimplePosition Int Int | AimPosition Int Int Int deriving (Show, Eq)
...
firstQuestion filename = do
  ...
  let SimplePosition horizontal depth = processFinalPosition initialPosition directions
...
secondQuestion filename = do
  ...
  let AimPosition horizontal depth aim = processFinalPosition initialPosition directions
  return (horizontal * depth)
...
processPosition (AimPosition h d aim) "forward" mag = AimPosition (h + mag) (d + (aim * mag)) aim 
