...
printCanvas coordSet (maxx, maxy) = putStr $ foldl
  (\s y -> s ++ "\n" ++ foldl
    (\s x -> s ++ (if (x, y) `member` coordSet then "#" else " "))
    ""
    [0 .. maxx]
  )
  "\n"
...
foldCoordinate ("x", v) (x, y) | x < v     = (x, y)
                               | otherwise = (v - (x - v), y)
foldCoordinate ("y", v) (x, y) | y < v     = (x, y)
                               | otherwise = (x, v - (y - v))
...
readCoordinates filename = do
  ...
  return
    ...
    , map
      ((\(axis : v : _) -> (axis, read v)) . splitOn "=" . head . tail . splitOn
        "fold along "
      )
      folds
    )
