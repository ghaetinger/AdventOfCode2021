...
firstQuestion filename = do
  ...
  let packets = (map (fst . readPacket . packetStringToBitList) . lines) content
...
processPacket (Operator _ 5 [p1, p2]) =
  if processPacket p1 > processPacket p2
    then 1
    else 0
...
readPacket (id1:id2:id3:tp1:tp2:tp3:0:content) =
  ...
  where
    ...
    (len, ncontent) =
      (Data.Bifunctor.first intListToNumber . splitAt 15) content
...
getOperatorNestedPackets content sz =
  (p, s) : getOperatorNestedPackets (drop s content) (sz - s)
  where
    (p, s) = readPacket content
