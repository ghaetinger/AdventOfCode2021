...
reduceUntilFinished fish =
  case reduce 0 fish of
    (_, _, False) ->
      case reduceOnlySplit fish of
...
reduceOnlySplit (Value a)
  | a >= 10 =
    ( Pair
        (Value ((floor . (/ 2) . fromIntegral) a))
        (Value ((ceiling . (/ 2) . fromIntegral) a))
    , Nothing
    , True)
  | otherwise = (Value a, Nothing, False)
reduceOnlySplit (Pair sa sb) =
  case reduceOnlySplit sa of
    (sf, Nothing, True) -> (Pair sf sb, Nothing, True)
    (_, _, False) ->
      case reduceOnlySplit sb of
        (sf, Nothing, True) -> (Pair sa sf, Nothing, True)
        (a, b, False)       -> (Value 0, b, False)
